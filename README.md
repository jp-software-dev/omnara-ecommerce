...
* **Full Purchase Lifecycle:** Persisted cart, guest and authenticated checkout, an order-confirmation screen that reconciles with the async webhook, automatic stock decrement with an inventory-movement audit trail, and a per-customer order history.
...
* **Admin/Vendor Operations Panel:** Product CRUD with category-specific fields, per-variant inventory adjustment with low-stock alerts, and order management — all gated by the same RLS policies that protect the public storefront.
* **Product Discovery:** Live search, wishlist, and a related-products module on every product page, driven by the same category graph as the catalog itself.
* **Customer Reviews:** Star ratings with a verified-purchase flag, one review per customer per product, and a running average surfaced on the product page.
* **Bilingual & Multi-Currency:** Full ES/EN interface via next-intl and a live MXN/USD switcher that recalculates every displayed price *and* the actual Stripe charge from a single exchange-rate source of truth — never just a display trick.
* **Original Visual Identity:** A warm stone-and-gold design system (Rubik/Nunito Sans) with deliberate interaction details — skip-to-content link, visible focus states, button press feedback, and full `prefers-reduced-motion` support.
